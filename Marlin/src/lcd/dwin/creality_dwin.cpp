...
void CrealityDWINClass::Draw_Print_Filename(bool reset/*=false*/) {
  ...
  if (process == Print) {
    ...
    if (slen > maxlen) {
      ...
      DWIN_Draw_String(false, false, DWIN_FONT_MENU, Color_White, Color_Bg_Black, npos, 60, outstr);  
  }
}
...
void CrealityDWINClass::Draw_Status_Area(bool icons/*=false*/) {
  ...
  if(icons) DWIN_Draw_Rectangle(1, Color_Bg_Black, 0, STATUS_Y, DWIN_WIDTH, DWIN_HEIGHT - 1);
    ...
    if (icons) {
      ...
      DWIN_Draw_String(false, false, DWIN_FONT_STAT, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 195 + 5 * STAT_CHR_W + 2, 384, F("%")); // mmm
    }
  ...
  // mmm    DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 3, 195 + 2 * STAT_CHR_W, 384, thermalManager.fan_speed[0]);
    ...
    if (zoffsetvalue != offset) {
      ...
      DWIN_Draw_String(false, true, DWIN_FONT_MENU, GetColor(eeprom_settings.status_area_text, Color_White), Color_Bg_Black, 205, 419, zoffsetvalue < 0 ? F("-") : F(" "));
    }
  #endif
